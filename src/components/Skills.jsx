...
export const Skills = () => {
  ...
  const categoryIcons = {
    'QA & Testing Specialties': (
      <Bug className="w-4 h-4 text-emerald-400" />
    ),
    'Frontend & Technical Literacy': (
  ...
  const allCategories = [
    'All',
    ...PROFILE_DATA.skillCategories.map((c) => c.category),
  ];

  const filteredCategories = PROFILE_DATA.skillCategories
    .map((cat) => {
    ...
    .filter(
      (cat) =>
        cat.isCategoryMatch && cat.filteredSkills.length > 0
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-3 text-base text-slate-400">
            A comprehensive matrix of manual testing, API validation,
            modern frontend technologies, and agile methodologies.
          </p>
