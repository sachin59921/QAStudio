...
export const QAShowcase: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'workflow' | 'defect-report' | 'postman'>('workflow');
  const [reportView, setReportView] = useState<'frontend-savvy' | 'generic'>('frontend-savvy');
  ...
  const endpoints = [
    {
      ...
      assertions: [
        ...
        { name: "Response body contains valid user claims & roles", passed: true },
        { name: "JWT payload has active expiration timestamp", passed: true },
    ...
    },
    {
      ...
      time: "62ms",
      assertions: [
        { name: "Catches missing CVV before payment gateway trigger", passed: true },
        { name: "Returns standard validation schema with error key", passed: true },
        { name: "Prevents UI race condition with 422 status code", passed: true }
      ],
    ...
    },
    {
      ...
      assertions: [
        { name: "Status code is 200 OK", passed: true },
        { name: "Response array size does not exceed specified limit (25)", passed: true },
        { name: "Total count header is present for frontend pagination", passed: true }
      ],
  ...
  return (
    <section id="methodology" className="py-20 bg-slate-900/50 border-t border-slate-800 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="mt-3 text-base text-slate-400">
            Explore my end-to-end testing lifecycle, see why developers prefer my bug reports, and inspect sample API test suites.
          </p>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900/80 to-slate-950 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <h4 className="text-white font-bold text-base flex items-center gap-2">
                  ...
                  <span>Proactive Defect Interception vs Reactive Bug Catching</span>
                </h4>
                <p className="text-xs sm:text-sm text-slate-400 max-w-3xl">
                  By reviewing acceptance criteria early and validating API payloads before the UI integration is complete, I prevent breaking changes from propagating through the release pipeline.
                </p>
              </div>
        ...
        {activeTab === 'defect-report' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-center gap-3">
              <span className="text-xs text-slate-400 font-mono">Select Report Comparison:</span>
            ...
            {reportView === 'frontend-savvy' ? (
              <div className="p-6 sm:p-8 rounded-2xl bg-slate-900/90 border border-emerald-500/30 shadow-2xl relative overflow-hidden">
                <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-800">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-1 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20 font-mono text-xs font-bold">
                      {PROFILE_DATA.sampleBugReport.id} • {PROFILE_DATA.sampleBugReport.severity}
                    </span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <ol className="space-y-2 text-xs sm:text-sm text-slate-300 list-decimal list-inside bg-slate-950/60 p-4 rounded-xl border border-slate-800">
                        {PROFILE_DATA.sampleBugReport.stepsToReproduce.map((step, i) => (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="p-3 rounded-xl bg-emerald-950/20 border border-emerald-500/20">
                        <div className="text-[11px] font-mono text-emerald-400 font-semibold mb-1 flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" /> Expected Behavior
                        </div>
                      ...
                      <div className="p-3 rounded-xl bg-rose-950/20 border border-rose-500/20">
                        <div className="text-[11px] font-mono text-rose-400 font-semibold mb-1 flex items-center gap-1">
                          <XCircle className="w-3 h-3" /> Actual Behavior
                        </div>
                        <p className="text-xs text-slate-300 leading-snug">
                  ...
                  {/* Right Column: The Developer Advantage (Console, API Payload & Root Cause Clue) */}
                  <div className="space-y-4">
                    <div className="p-4 rounded-xl bg-slate-950 border border-slate-800">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-mono text-teal-400 flex items-center gap-1.5 font-semibold">
                          <Code2 className="w-3.5 h-3.5" /> Developer Root-Cause Insight:
                        </span>
                        <span className="text-[10px] text-slate-500 font-mono">React / Redux Context</span>
                      </div>
                    ...
                    <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 font-mono text-xs">
                      <div className="flex items-center justify-between text-slate-400 pb-2 mb-2 border-b border-slate-800">
                        <span className="text-xs text-amber-400 flex items-center gap-1">
                          <Terminal className="w-3 h-3" /> Network Payload Inspection
                        </span>
                        <span className="text-[10px] text-slate-500">Postman / DevTools</span>
                      </div>
              ...
              <div className="p-6 sm:p-8 rounded-2xl bg-slate-900/60 border border-rose-500/20 opacity-80">
                <div className="flex items-center gap-3 pb-3 border-b border-slate-800 text-rose-400 text-xs font-mono">
                  ...
                  <span>Generic / Incomplete Bug Report (What developers usually complain about)</span>
                </div>
                <div className="mt-4 space-y-3">
                  ...
                  <div className="p-4 rounded-xl bg-slate-950/70 border border-slate-800 text-xs text-slate-400 space-y-2">
                    <p><strong>Description:</strong> I tried to pay for an order and the button stayed loading forever. Please fix asap.</p>
                    <p><strong>Expected:</strong> It should work.</p>
                    <p><strong>Steps:</strong> Click the button.</p>
                    <p className="text-rose-400 italic">No environment details, no console logs, no network status codes, no expected vs actual comparison.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
              ...
              <div className="flex flex-wrap gap-2">
                {endpoints.map((ep, idx) => (
                  ...
                  >
                    <span className="text-amber-400 font-bold mr-1">{ep.method}</span>
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 p-3 rounded-xl bg-slate-900/90 border border-slate-800">
              <div className="flex items-center gap-3 overflow-hidden font-mono text-xs">
                <span className={`px-2.5 py-1 rounded font-bold ${
                  endpoints[selectedEndpoint].method === 'POST' ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'
                }`}>
              ...
              <div className="flex items-center gap-3 self-end md:self-center">
                <span className={`text-xs font-mono font-bold px-2 py-1 rounded ${
                  ...
                  {endpoints[selectedEndpoint].status} {endpoints[selectedEndpoint].statusText}
                </span>
                ...
                >
                  <Play className={`w-3.5 h-3.5 ${apiRunning ? 'animate-spin' : ''}`} />
                  <span>{apiRunning ? 'Running...' : 'Run Test'}</span>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-xs font-mono uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-1.5">
                  ...
                  <span>Postman Test Scripts (Tests Passed: {endpoints[selectedEndpoint].assertions.length}/{endpoints[selectedEndpoint].assertions.length})</span>
                </h4>
