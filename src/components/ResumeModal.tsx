...
import { X, Printer, Download, Mail, Phone, MapPin, Linkedin, Globe, CheckCircle2, ExternalLink } from 'lucide-react';
      ...
      <div className="relative w-full max-w-4xl bg-white text-slate-900 rounded-2xl shadow-2xl overflow-hidden my-auto border border-slate-200">
        ...
        <div className="flex items-center justify-between px-6 py-3 bg-slate-900 text-white border-b border-slate-800 print:hidden">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-sm">Resume Document • Sachin Kumar Pal</span>
            <span className="text-xs text-slate-400">Software QA Engineer</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
        ...
        <div className="p-8 sm:p-10 max-h-[85vh] overflow-y-auto print:max-h-none print:p-0 print:overflow-visible font-sans text-slate-800">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              ...
              <div className="text-xs space-y-1 text-slate-700 font-mono text-left sm:text-right shrink-0">
                ...
                <div className="flex items-center sm:justify-end gap-1.5">
                  ...
                  <span>linkedin.com/in/{PROFILE_DATA.linkedinUsername}</span>
                </div>
                <div className="flex items-center sm:justify-end gap-1.5">
          ...
          {/* Core Testing Skills & Competencies */}
          <div className="mb-6">
            <h2 className="text-xs uppercase font-extrabold tracking-widest text-slate-950 border-b border-slate-300 pb-1 mb-2 font-mono">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700">
              <div>
                <strong className="text-slate-900">QA &amp; Testing:</strong> Manual Testing, API Testing (Postman), Black Box Testing, Smoke Testing, UAT, Functional &amp; Regression Testing, Test Case Design &amp; Matrices, Bug Lifecycle Management.
              </div>
              <div>
                <strong className="text-slate-900">Frontend &amp; Dev:</strong> HTML, CSS, JavaScript, React.js, Browser DevTools, Mobile Web Testing.
              </div>
              <div>
                <strong className="text-slate-900">Methodologies:</strong> Agile/Scrum, Sprint Planning, Stand-ups, Retrospectives, Defect Triage.
              </div>
              <div>
                <strong className="text-slate-900">Cloud &amp; GenAI:</strong> Google Cloud Computing Foundations, Cloud Speech API, App Engine, Gemini &amp; Streamlit Apps.
              </div>
            </div>
          </div>
              ...
              {PROFILE_DATA.experiences.map((exp) => (
                <div key={exp.id} className="text-xs">
                  <div className="flex items-center justify-between font-bold text-slate-900">
                    <span>{exp.company} — <span className="text-emerald-700 font-semibold">{exp.role}</span></span>
                    <span className="font-mono text-slate-600 font-normal">{exp.period} ({exp.location})</span>
                  </div>
                  {exp.description && (
                    <p className="text-slate-700 mt-1 italic">{exp.description}</p>
                  )}
            ...
            <div className="space-y-2 text-xs">
              {PROFILE_DATA.education.map((edu) => (
                <div key={edu.id} className="flex items-start justify-between">
                  ...
                  <span className="font-mono text-slate-600 font-normal shrink-0 ml-2">{edu.period}</span>
                </div>
              ))}
            </div>
          </div>
